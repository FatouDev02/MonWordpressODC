...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    class Kushak_Custom_Radio_Image_Control extends WP_Customize_Control {
...
// Radio COlor Schema
class Kushak_Custom_Radio_Color_Schema extends WP_Customize_Control {
 ...
 * @access public
 */
class Kushak_Customize_Section_Upsell extends WP_Customize_Section {
...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    // Repeator Info
    class Kushak_Premium_Notiece_Control extends WP_Customize_Control {

        public $type = 'kushak-premiuminfonotice';
            ...
            <span class="customize-control-title">
                <div class="theme-info-icon">
                    <div class="icon-notice-wrap">
                        ...
                        <span><?php echo esc_html__('More ','kushak'). esc_html( $this->label ). esc_html__(' Available on Premium Version.', 'kushak' ); ?></span>
                    </div>
                </div>
            </span>
...
*/
class Kushak_Repeater_Controler extends WP_Customize_Control {
    /**
     * The control type.
     *
     * @access public
    ...
    */
    ...
    public $kushak_box_label = '';

    public $kushak_box_add_control = '';

    public $kushak_box_add_button = '';
    ...
    public function __construct( $manager, $id, $args = array(), $fields = array() ) {
        $this->fields = $fields;
        $this->kushak_box_label = $args['kushak_box_label'] ;
        $this->kushak_box_add_control = $args['kushak_box_add_control'];
        $this->kushak_box_add_button = $args['kushak_box_add_button'];
    ...
    public function render_content() {
        ...
        <ul class="kushak-repeater-field-control-wrap">
            <?php
            $this->kushak_get_fields();
            ?>
        </ul>
        <input type="hidden" <?php esc_attr( $this->link() ); ?> class="kushak-repeater-collector" value="<?php echo esc_attr( $this->value() ); ?>" />
        <?php

        if( isset( $this->kushak_box_add_button ) && $this->kushak_box_add_button ){ ?>
            
            <button type="button" class="button kushak-add-control-field"><?php echo esc_html( $this->kushak_box_add_control ); ?></button>
        <?php
        }
    ...
    private function kushak_get_fields(){
        ...
        ?>
        <li class="kushak-repeater-field-control">

        <div class="title-rep-wrap">
            <span class="dashicons dashicons-move twp-filter-icon"></span>
            <h3 class="kushak-repeater-field-title"><?php echo esc_html( $this->kushak_box_label ); ?></h3>
            <span class="dashicons dashicons-arrow-down twp-filter-icon"></span>
        </div>

        <div class="kushak-repeater-fields">
        <?php
            foreach ($fields as $key => $field) {
            $class = isset($field['class']) ? $field['class'] : '';
            ?>
            <div class="kushak-fields kushak-type-<?php echo esc_attr($field['type']).' '.esc_attr( $class ); ?>">
                <?php 
                    ...
                    switch ($field['type']) {
                        ...
                        case 'selector_color':
                        $options = $field['options'];
                            echo '<div class="kushak-type-radio">';
                        ...
                        case 'upload':
                                ...
                                esc_html_e('No image selected', 'kushak');
                                echo '</div>';
                                echo '<div class="thumbnail thumbnail-image">';
                                echo $image;
                                echo '</div>';
                                echo '<div class="actions clearfix">';
                                echo '<button type="button" class="button theme-image-delete align-left">'.esc_html__('Remove', 'kushak').'</button>';
                                echo '<button type="button" class="button theme-custom-upload-button alignright">'.esc_html__('Select Image', 'kushak').'</button>';
            ...
            <div class="clearfix kushak-repeater-footer">
                <div class="alignright">
                <?php if( isset( $this->kushak_box_add_button ) && $this->kushak_box_add_button ){ ?>
                    <a class="kushak-repeater-field-remove" href="#remove"><?php esc_html_e('Delete', 'kushak') ?>|</a> 
                <?php } ?>
                <a class="kushak-repeater-field-close" href="#close"><?php esc_html_e('Close', 'kushak') ?></a>
                </div>
            </div>
        </div>
        </li>
...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    // Radio Image Custom Control Class.
    class Kushak_Separator extends WP_Customize_Control {

        public $type = 'always-separator';
