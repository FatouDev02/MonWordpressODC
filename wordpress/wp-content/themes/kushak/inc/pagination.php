...
if( !function_exists('kushak_archive_pagination_x') ):

	// Archive Page Navigation
	function kushak_archive_pagination_x(){
		...
		$kushak_pagination_layout = esc_html( get_theme_mod( 'kushak_pagination_layout',$kushak_default['kushak_pagination_layout'] ) );
		$kushak_pagination_load_more = esc_html__('Load More Posts','kushak');
		$kushak_pagination_no_more_posts = esc_html__('No More Posts','kushak');

		if( $kushak_pagination_layout == 'next-prev' ){

            the_posts_navigation([
            	'prev_text'=>kushak_the_theme_svg( 'arrow-left',$return = true ).'<span class="nav-link-label">'. __('Older posts','kushak').'</span>',
				'next_text'=> '<span class="nav-link-label">'. __('Newer posts','kushak').'</span>'.kushak_the_theme_svg( 'arrow-right',$return = true ),
            ]);

		}elseif( $kushak_pagination_layout == 'load-more' || $kushak_pagination_layout == 'auto-load' ){ ?>
			...
			<div class="theme-ajax-loadpost hide-no-js">
				...
				<?php if( $max > 1 ){ ?>
					...
					<button class="theme-loading-button theme-loading-style" href="javascript:void(0)">
						...
						<span class="loading-text"><?php echo esc_html( $kushak_pagination_load_more ); ?></span>
					</button>
				...
				<?php }else{ ?>
					...
					<button class="theme-loading-button theme-loading-style theme-no-posts" href="javascript:void(0)">
						<span style="display: none;" class="theme-loading-status"></span>
						<span class="loading-text"><?php echo esc_html( $kushak_pagination_no_more_posts ); ?></span>
					</button>
...
add_action('kushak_archive_pagination','kushak_archive_pagination_x',20);


add_action('wp_ajax_kushak_single_infinity', 'kushak_single_infinity_callback');
add_action('wp_ajax_nopriv_kushak_single_infinity', 'kushak_single_infinity_callback');

// Recommendec Post Ajax Call Function.
function kushak_single_infinity_callback() {

    if ( isset( $_POST['_wpnonce'] ) && wp_verify_nonce( wp_unslash( $_POST['_wpnonce'] ), 'kushak_ajax_nonce' ) ) {
            ...
            while ( $post_single_next_posts->have_posts() ) :
                ...
                <article id="post-<?php the_ID(); ?>" <?php post_class('after-load-ajax-'.get_the_ID() ); ?>> 
				    ...
				    if( has_post_thumbnail() ){
				        ...
				        if( empty( $kushak_ed_feature_image ) ){ ?>
				            ...
				            <div class="entry-featured-thumbnail mb-20">
					            ...
					            <?php if( class_exists('Booster_Extension_Class') && ( empty( $kushak_ed_post_views ) || empty( $kushak_ed_post_read_time ) ) ){ ?>
					...
					<div class="post-content-wrap">
						...
						<?php if( empty( $kushak_ed_post_social_share ) && class_exists( 'Booster_Extension_Class' ) && 'post' === get_post_type() ){ ?>
						...
						<div class="post-content">
							...
							<div class="entry-content">
								...
								if( class_exists('Booster_Extension_Class') && empty( $kushak_ed_post_like_dislike ) ){ 
