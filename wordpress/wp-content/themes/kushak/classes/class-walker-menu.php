...
if (!class_exists('Kushak_Walker_Page')) {
    /**
     * CUSTOM PAGE WALKER
     * A custom walker for pages.
     */
    class Kushak_Walker_Page extends Walker_Page
    {
        ...
        public function start_el(&$output, $page, $depth = 0, $args = array(), $current_page = 0)
        {
            ...
            if ('' === $page->post_title) {
                ...
                $page->post_title = sprintf(__('#%d (no title)', 'kushak'), $page->ID);
            }
            ...
            if (isset($args['show_sub_menu_icons']) && true === $args['show_sub_menu_icons']) {
                if (isset($args['pages_with_children'][$page->ID])) {
                    $args['icon_rennder'] = '<span class="icon">'.kushak_the_theme_svg('arrow-down',true).'</span>';
                }
            }
            ...
            if (isset($args['show_toggles']) && true === $args['show_toggles']) {
                if (isset($args['pages_with_children'][$page->ID])) {
                    ...
                    $args['list_item_after'] = '<button class="toggle submenu-toggle" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="250" aria-expanded="false"><span class="screen-reader-text">' . __( 'Show sub menu', 'kushak' ) . '</span>' . kushak_get_theme_svg( 'arrow-down' ) . '</button>';
                }
            }
